...
<div class="titleArea clearfix">
	<div class="wrapper col-md-12">
		...
		<div class="horControlB menu_action">
			<ul>
				<li>
					<a href="<?php echo admin_url('room_type/create');?>">
			...
			<table class="table sTable mTable myTable" id="checkAll">
					...
					<tr>
						<td colspan="9">
							<div class="list_action itemActions">
								<a href="javascript:void(0)" onclick = "deleteAll()" id="submit" class="button blueB" url="<?php echo admin_url('room_type/deleteAll');?>">
				...
				<tbody class="list_item">
					<?php
						...
						foreach ($list as $row) {				
					?>
				    <tr class='row_<?php echo $row->room_type_id;?>'>
				    	<td class="textC">
						...
						<td class="textC" id="status">
							...
							if($row->status == 1){
							?>
								<a href="javascript:void(0)" onclick="status(<?php echo $row->room_type_id;?>)" class="lightbox" title="block">
									<img src="<?php echo base_url();?>public/admin/images/icons/color/tick.png" />
								</a>
							<?php
							}else{
							?>
								<a href="javascript:void(0)" onclick="status(<?php echo $row->room_type_id;?>)" class="lightbox" title="active">
						...
						<td class="textC">
							<a href="<?php echo base_url('admin/room_type/edit/'.$row->room_type_id);?>" class="lightbox" title="edit">
								<img src="<?php echo base_url();?>public/admin/images/icons/color/pencil.png" />
							</a>
							&nbsp;
							<a href="<?php echo base_url('admin/room_type/delete/'.$row->room_type_id);?>" class="lightbox" title="delete" onclick="return confirm('Bạn có muốn xóa?');">
								<img src="<?php echo base_url();?>public/admin/images/icons/color/uninstall.png" />
							</a>
						</td>
						<td class="textC"><?php echo $row->room_type_id;?></td>
					</tr>	
					<?php
						$i++;
					}
...
<script type="text/javascript">
	...
	function status(id){
		...
		$.ajax({
            url : admin_url+'/room_type/status',
            type : "post",
            dataType:"text",
            data : {
                 'id' : id,
